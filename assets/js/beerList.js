function makeTapList(beers){
	...
	$.each(beers, function(key, beer){
		/*if ( beer.location.status.toLowerCase() == 'serving' ) {*/
			...
			element += '<h5 class=\"card-title\">'+beer.brewer.name+'</h5>';
			element += '<h4 class=\"card-subtitle mb-2 text-muted\">'+beer.name+'</h4>';
			element += '<p class=\"card-text\">'+beer.style.name+' &mdash; ';
			element += '<strong>'+beer.abv+'% ABV</strong></p>';
			element += '<p class=\"text-muted\">'+beer.brewer.location;
			element += '<img class=\"float-right\" src=\"./assets/images/'+locationclass[beer.location.id]+'.png\"></img>';
			element += '</p></div>';
